...
(function () {
  ...
  var SHARED_UNIFORMS = {
    ...
    uFadeNear:   { value: 4.0 },
    uFadeFar:    { value: 22.0 },
    uBgColor:    { value: new THREE.Color(0x000000) }
  };
  ...
  function makeCircleMaterial(opts) {
    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: [
        ...
        '  float shade = mix(0.25, 1.0, vBrightness);',
  ...
  var THUMB_URLS = [
    ...
    'https://pub-eda9762ae69b42e8a6152cd19a5cc486.r2.dev/On%20Running/Cover-Homepage-RacefortheFeeling.gif',
  ...
  function init(positions, luminance) {
    ...
    var material = makeCircleMaterial({
      color: 0xffffff,
      size: 0.01,
      opacity: MODE === 'background' ? 0.85 : 0.9
    });
    ...
    var OVERVIEW_POS    = new THREE.Vector3(maxDim * 0.55, maxDim * 0.30, maxDim * 0.55);
    ...
    function pickRedVertices() {
      ...
      // Pixel-level coverage: how many points project to the SAME 4x4-pixel
      // tile. A tile with many overlapping points renders solid white; a
      // tile with few/no points renders black. We want vertices whose pixel
      // is brightly lit.
      var TILE = 4;
      var tcols = Math.ceil(W / TILE);
      var trows = Math.ceil(H / TILE);
      var coverage = new Uint16Array(tcols * trows);
      var tileOf = new Int32Array(n);
      for (var j = 0; j < n; j++) {
        if (!visible[j]) { tileOf[j] = -1; continue; }
        var tx = Math.floor(sx[j] / TILE);
        var ty = Math.floor(sy[j] / TILE);
        var ti = ty * tcols + tx;
        tileOf[j] = ti;
        if (coverage[ti] < 65535) coverage[ti]++;
      }

      // A vertex is "on a white pixel" if its tile has high coverage —
      // many points piling up on the same pixel = bright white.
      var maxCov = 0;
      for (var c = 0; c < coverage.length; c++) if (coverage[c] > maxCov) maxCov = coverage[c];
      var minCov = Math.max(4, Math.floor(maxCov * 0.4)); // top 60% of brightest tiles

      // Build candidates: visible vertex on a bright tile, with safe margins.
      var marginX = W * 0.08;
      var marginTop = H * 0.10;
      var marginBottom = H * 0.18;
      var candidates = [];
      for (var p = 0; p < n; p++) {
        if (!visible[p]) continue;
        if (coverage[tileOf[p]] < minCov) continue;
        if (sx[p] < marginX || sx[p] > W - marginX) continue;
        if (sy[p] < marginTop || sy[p] > H - marginBottom) continue;
        candidates.push(p);
      }
      console.log('[pickRedVertices] maxCov=' + maxCov + ' minCov=' + minCov +
                  ' candidates=' + candidates.length);

      if (candidates.length < 5) {
        // Fallback: just take the 5 vertices on the brightest tiles
        candidates = [];
        for (var q = 0; q < n; q++) if (visible[q]) candidates.push(q);
        candidates.sort(function (a, b) { return coverage[tileOf[b]] - coverage[tileOf[a]]; });
        return candidates.slice(0, 5);
      }

      // Spread the 5 picks across the visible bright mass via farthest-point
      // sampling in screen space. Seed with the brightest candidate.
      var seed = candidates[0];
      for (var s = 1; s < candidates.length; s++) {
        if (coverage[tileOf[candidates[s]]] > coverage[tileOf[seed]]) seed = candidates[s];
      }
      var chosen = [seed];
      var minDist = new Float32Array(candidates.length);
      for (var t = 0; t < candidates.length; t++) {
        var ct = candidates[t];
        var dx = sx[ct] - sx[seed], dy = sy[ct] - sy[seed];
        minDist[t] = Math.sqrt(dx*dx + dy*dy);
      }
      while (chosen.length < 5) {
        var best = -1, bestD = -1;
        for (var u = 0; u < candidates.length; u++) {
          if (minDist[u] > bestD) { bestD = minDist[u]; best = u; }
        }
        var pickIdx = candidates[best];
        chosen.push(pickIdx);
        for (var w2 = 0; w2 < candidates.length; w2++) {
          var cw = candidates[w2];
          var dx2 = sx[cw] - sx[pickIdx], dy2 = sy[cw] - sy[pickIdx];
          var nd = Math.sqrt(dx2*dx2 + dy2*dy2);
          if (nd < minDist[w2]) minDist[w2] = nd;
        }
      }
      return chosen;
    }
    ...
    SHARED_UNIFORMS.uBgColor.value.set(0x000000);
    ...
    // ===== INTERACTIVE MODE =====
    // STATE_COUNT = 1 overview + 5 projects = 6
    var STATE_COUNT = 6;
